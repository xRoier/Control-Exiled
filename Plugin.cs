...
namespace Control
{
    public class Plugin : Plugin<Config>
    {
        ...
        private List<string> _staffOnline = new List<string>();
        ...
        public override void OnEnabled()
        {
            ...
            Exiled.Events.Handlers.Player.Kicking += OnKicked;
        ...
        private void OnWaiting()
        {
            ...
            _coroutines.Add(Timing.RunCoroutine(StaffActivity()));
        }
        ...
        private void OnVerified(VerifiedEventArgs ev)
        {
            ...
            if (Config.staffGroups.Contains(ev.Player.GroupName))
            {
                ws?.Send($"11 0 {ev.Player.UserId} {ev.Player.Nickname}");
                _staffOnline.Add(ev.Player.UserId);
            }
        ...
        private void OnDestroying(DestroyingEventArgs ev)
        {
            ...
            if (_staffOnline.Contains(ev.Player.UserId))
                _staffOnline.Remove(ev.Player.UserId);
        ...
        // Staff Activity
        public IEnumerator<float> StaffActivity()
        {
            for (;;)
            {
                foreach (var staff in _staffOnline)
                {
                    ws?.Send($"11 1 {staff}");
                }
